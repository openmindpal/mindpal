...
async function orchestratorTurn(conversationId) {
  const res = await fetch(`${apiBase}/orchestrator/turn`, {
    ...
    body: JSON.stringify({ message: "打开笔记并新建一条", conversationId: conversationId ?? undefined }),
  });
  const json = await res.json().catch(() => null);
...
async function publishEntityCreateTool() {
  const res = await fetch(`${apiBase}/tools/entity.create/publish`, {
    method: "POST",
    headers: { ...headers(), "content-type": "application/json", "x-trace-id": "t-web-e2e-tool-pub-entity-create" },
    body: JSON.stringify({
      scope: "write",
      resourceType: "entity",
      action: "create",
...
const entityCreateRef = await publishEntityCreateTool();
assert(entityCreateRef, "expected_entity_create_tool_published");
