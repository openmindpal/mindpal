...
export default function TaskDetailClient(props: { locale: string; taskId: string; initialTask: unknown; initialTaskStatus: number; initialMessages: unknown; initialMessagesStatus: number }) {
  ...
  const initialError = useMemo(() => {
    ...
    return "";
  }, [msgData, msgStatus, props.locale, taskData, taskStatus]);

  async function refresh() {
    setError("");
    const tRes = await fetch(`${API_BASE}/tasks/${encodeURIComponent(props.taskId)}`, { headers: apiHeaders(props.locale), cache: "no-store" });
    setTaskStatus(tRes.status);
    const tJson: unknown = await tRes.json().catch(() => null);
    setTaskData((tJson as TaskDetailResp) ?? null);
    const mRes = await fetch(`${API_BASE}/tasks/${encodeURIComponent(props.taskId)}/messages?limit=50`, { headers: apiHeaders(props.locale), cache: "no-store" });
  ...
  async function continueAgentRun(runId: string) {
    await runAction(async () => {
      const res = await fetch(`${API_BASE}/tasks/${encodeURIComponent(props.taskId)}/agent-runs/${encodeURIComponent(runId)}/continue`, {
        method: "POST",
  ...
  async function sendHandoff() {
    ...
    await runAction(async () => {
      const res = await fetch(`${API_BASE}/tasks/${encodeURIComponent(props.taskId)}/messages`, {
        method: "POST",
