...
export default function ApprovalDetailClient(props: { locale: string; approvalId: string; initial: unknown; initialStatus: number }) {
  ...
  async function refresh() {
    setError("");
    const res = await fetch(`${API_BASE}/approvals/${encodeURIComponent(props.approvalId)}`, { headers: apiHeaders(props.locale), cache: "no-store" });
  ...
  async function decide(decision: "approve" | "reject") {
    ...
    setBusy(true);
    try {
      const res = await fetch(`${API_BASE}/approvals/${encodeURIComponent(props.approvalId)}/decisions`, {
        method: "POST",
