...
export default function ChangeSetsClient(props: { locale: string; initial: unknown; initialStatus: number; initialPipelines: unknown; initialPipelinesStatus: number }) {
  ...
  async function refresh() {
    ...
    const res = await fetch(`${API_BASE}/governance/changesets?${q.toString()}`, { headers: apiHeaders(props.locale), cache: "no-store" });
    ...
    const pRes = await fetch(`${API_BASE}/governance/changesets/pipelines?${q.toString()}&mode=full`, { headers: apiHeaders(props.locale), cache: "no-store" });
  ...
  async function create() {
    ...
    try {
      const canaryTargets = canaryTargetsText
        ...
        .filter(Boolean)
        .slice(0, 50);
      const res = await fetch(`${API_BASE}/governance/changesets`, {
        method: "POST",
