...
export default function RetrievalLogsClient(props: { locale: string }) {
  ...
  async function refresh() {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/governance/knowledge/retrieval-logs?${q.toString()}`, { headers: apiHeaders(props.locale), cache: "no-store" });
      ...
      <Table header={<span>{rows.length ? `${rows.length}` : "-"}</span>}>
        <thead>
          <tr>
            ...
            <th align="left">candidateCount</th>
            <th align="left">returnedCount</th>
            <th align="left">degraded</th>
            <th align="left">rankPolicy</th>
