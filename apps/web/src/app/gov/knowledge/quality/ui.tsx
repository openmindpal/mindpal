...
export default function KnowledgeQualityClient(props: { locale: string }) {
  ...
  async function refresh(selectedEvalSetId?: string) {
    ...
    setBusy(true);
    try {
      const sRes = await fetch(`${API_BASE}/governance/knowledge/quality/eval-sets?limit=50`, { headers: apiHeaders(props.locale), cache: "no-store" });
      ...
      const rRes = await fetch(`${API_BASE}/governance/knowledge/quality/runs?${q.toString()}`, { headers: apiHeaders(props.locale), cache: "no-store" });
  ...
  async function createSet() {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/governance/knowledge/quality/eval-sets`, {
        method: "POST",
  ...
  async function runEval(evalSetId: string) {
    ...
    setBusy(true);
    try {
      const res = await fetch(`${API_BASE}/governance/knowledge/quality/eval-sets/${encodeURIComponent(evalSetId)}/runs`, {
        method: "POST",
