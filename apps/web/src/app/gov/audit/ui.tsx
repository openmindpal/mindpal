...
export default function AuditClient(props: { locale: string }) {
  ...
  const [retentionStatus, setRetentionStatus] = useState<number>(0);
  ...
  const [holdsStatus, setHoldsStatus] = useState<number>(0);
  ...
  const [exportsStatus, setExportsStatus] = useState<number>(0);
  const [exports, setExports] = useState<AuditExportRow[]>([]);

  const [siemStatus, setSiemStatus] = useState<number>(0);
  ...
  async function downloadArtifact(artifactId: string) {
    ...
    setBusy(true);
    try {
      const res = await fetch(`${API_BASE}/artifacts/${encodeURIComponent(artifactId)}/download-token`, {
        method: "POST",
  ...
  async function search() {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/audit?${q.toString()}`, { headers: apiHeaders(props.locale), cache: "no-store" });
  ...
  async function verify() {
    ...
    try {
      ...
      if (Number.isFinite(n) && n > 0) q.set("limit", String(n));
      const res = await fetch(`${API_BASE}/audit/verify?${q.toString()}`, { headers: apiHeaders(props.locale), cache: "no-store" });
  ...
  async function loadRetention() {
    ...
    setBusy(true);
    try {
      const res = await fetch(`${API_BASE}/audit/retention`, { headers: apiHeaders(props.locale), cache: "no-store" });
      setRetentionStatus(res.status);
      const json: unknown = await res.json().catch(() => null);
      if (!res.ok) throw toApiError(json);
  ...
  async function saveRetention() {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/audit/retention`, {
        method: "PUT",
        headers: { ...apiHeaders(props.locale), "content-type": "application/json" },
  ...
  async function loadHolds() {
    ...
    setBusy(true);
    try {
      const res = await fetch(`${API_BASE}/audit/legal-holds?limit=50`, { headers: apiHeaders(props.locale), cache: "no-store" });
      setHoldsStatus(res.status);
      const json: unknown = await res.json().catch(() => null);
      if (!res.ok) throw toApiError(json);
  ...
  async function createHold() {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/audit/legal-holds`, {
        method: "POST",
        headers: { ...apiHeaders(props.locale), "content-type": "application/json" },
  ...
  async function releaseHold(holdId: string) {
    ...
    setBusy(true);
    try {
      const res = await fetch(`${API_BASE}/audit/legal-holds/${encodeURIComponent(holdId)}/release`, {
        method: "POST",
  ...
  async function loadExports() {
    ...
    setBusy(true);
    try {
      const res = await fetch(`${API_BASE}/audit/exports?limit=50`, { headers: apiHeaders(props.locale), cache: "no-store" });
      setExportsStatus(res.status);
      const json: unknown = await res.json().catch(() => null);
      if (!res.ok) throw toApiError(json);
  ...
  async function loadSiemDestinations() {
    ...
    try {
      const res = await fetch(`${API_BASE}/audit/siem-destinations?limit=50`, { headers: apiHeaders(props.locale), cache: "no-store" });
      setSiemStatus(res.status);
      const json: unknown = await res.json().catch(() => null);
      if (!res.ok) throw toApiError(json);
  ...
  async function createSiemDestination() {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/audit/siem-destinations`, {
        method: "POST",
        headers: { ...apiHeaders(props.locale), "content-type": "application/json" },
        body: JSON.stringify({
          name: siemName.trim(),
  ...
  async function testSiemDestination(id: string) {
    ...
    setSiemLastResult(null);
    try {
      const res = await fetch(`${API_BASE}/audit/siem-destinations/${encodeURIComponent(id)}/test`, { method: "POST", headers: apiHeaders(props.locale) });
      setSiemStatus(res.status);
      const json: unknown = await res.json().catch(() => null);
      if (!res.ok) throw toApiError(json);
  ...
  async function backfillSiemDestination(id: string) {
    ...
    setSiemLastResult(null);
    try {
      const res = await fetch(`${API_BASE}/audit/siem-destinations/${encodeURIComponent(id)}/backfill`, {
        method: "POST",
        headers: { ...apiHeaders(props.locale), "content-type": "application/json" },
  ...
  async function loadSiemDlq(destinationId: string) {
    ...
    try {
      const res = await fetch(`${API_BASE}/audit/siem-destinations/${encodeURIComponent(destinationId)}/dlq?limit=50`, { headers: apiHeaders(props.locale), cache: "no-store" });
  ...
  async function clearSiemDlq(destinationId: string) {
    ...
    setSiemDlqLastResult(null);
    try {
      const res = await fetch(`${API_BASE}/audit/siem-destinations/${encodeURIComponent(destinationId)}/dlq/clear`, { method: "POST", headers: apiHeaders(props.locale) });
      setSiemDlqStatus(res.status);
      const json: unknown = await res.json().catch(() => null);
      if (!res.ok) throw toApiError(json);
  ...
  async function requeueSiemDlq(destinationId: string) {
    ...
    setSiemDlqLastResult(null);
    try {
      const res = await fetch(`${API_BASE}/audit/siem-destinations/${encodeURIComponent(destinationId)}/dlq/requeue`, {
        method: "POST",
        headers: { ...apiHeaders(props.locale), "content-type": "application/json" },
  ...
  async function createExport() {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/audit/exports`, {
        method: "POST",
        headers: { ...apiHeaders(props.locale), "content-type": "application/json" },
        body: JSON.stringify(body),
      });
  ...
  async function refreshExport(exportId: string) {
    ...
    setBusy(true);
    try {
      const res = await fetch(`${API_BASE}/audit/exports/${encodeURIComponent(exportId)}`, { headers: apiHeaders(props.locale), cache: "no-store" });
      const json: unknown = await res.json().catch(() => null);
      if (!res.ok) throw toApiError(json);
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <>
            <Badge>{eventsStatus || "-"}</Badge>
            <button onClick={search} disabled={busy}>
              {t(props.locale, "gov.audit.search")}
            </button>
          </>
        }
      />
      ...
      <div style={{ marginTop: 16 }}>
        <Card
          ...
          footer={
            <span>
              status={retentionStatus || "-"} updatedAt={retentionUpdatedAt || "-"}
            </span>
          }
        >
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
      ...
      <div style={{ marginTop: 16 }}>
        <Card
          ...
          footer={
            <span>
              status={holdsStatus || "-"} count={holdRows.length}
            </span>
          }
        >
      ...
      <div style={{ marginTop: 16 }}>
        <Card
          ...
          footer={
            <span>
              status={exportsStatus || "-"} count={exportRows.length}
            </span>
          }
        >
          <div style={{ display: "grid", gap: 10 }}>
      ...
      <div style={{ marginTop: 16 }}>
        <Card
          ...
          footer={
            <span>
              status={siemStatus || "-"} count={siemRows.length}
            </span>
          }
        >
          <div style={{ display: "grid", gap: 10 }}>
              ...
              <Table header={<span>{t(props.locale, "gov.audit.siem.listTitle")}</span>}>
                <thead>
                  <tr>
                    ...
                    <th align="left">batchSize</th>
                    <th align="left">timeoutMs</th>
            ...
            {siemDlqDestId ? (
              <div style={{ display: "grid", gap: 10 }}>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                  ...
                  <span>status={siemDlqStatus || "-"}</span>
                  <span>count={siemDlqRows.length}</span>
                  ...
                  <Table header={<span>{t(props.locale, "gov.audit.siem.dlqListTitle")}</span>}>
                    <thead>
                      <tr>
                        ...
                        <th align="left">eventTs</th>
                        <th align="left">attempts</th>
                        <th align="left">createdAt</th>
                        <th align="left">lastErrorDigest</th>
                      </tr>
                    </thead>
                    <tbody>
                      {siemDlqRows.map((x) => (
        ...
        >
          <thead>
            <tr>
              ...
              <th align="left">subject_id</th>
              <th align="left">resource_type</th>
              <th align="left">action</th>
              <th align="left">result</th>
              <th align="left">trace_id</th>
            </tr>
          </thead>
          <tbody>
      ...
      <div style={{ marginTop: 16 }}>
        <Card
          ...
          footer={
            <span>
              status={verifyStatus || "-"} ok={String(verifyResult?.ok ?? "")} checkedCount={String(verifyResult?.checkedCount ?? "")} lastEventHash=
