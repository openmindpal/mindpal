...
export default function GovTriggersClient(props: { locale: string; initial: { status: number; json: any } }) {
  ...
  const refresh = useCallback(async () => {
    const res = await fetch(`${API_BASE}/governance/triggers?limit=50`, { headers: apiHeaders(props.locale), cache: "no-store" });
  ...
  async function createOne() {
    await runAction(async () => {
      ...
      const res = await fetch(`${API_BASE}/governance/triggers`, {
        method: "POST",
  ...
  async function preflight(triggerId: string) {
    await runAction(async () => {
      const res = await fetch(`${API_BASE}/governance/triggers/${encodeURIComponent(triggerId)}/preflight`, {
        method: "POST",
  ...
  async function manualFire(triggerId: string) {
    await runAction(async () => {
      const res = await fetch(`${API_BASE}/governance/triggers/${encodeURIComponent(triggerId)}/fire`, {
        method: "POST",
        headers: { ...apiHeaders(props.locale), "content-type": "application/json" },
        body: JSON.stringify({}),
      });
  ...
  return (
    <div style={{ padding: 24, display: "grid", gap: 16 }}>
      <PageHeader title={t(props.locale, "gov.triggers.title")} description={t(props.locale, "gov.triggers.desc")} actions={<Badge>{data.status}</Badge>} />
