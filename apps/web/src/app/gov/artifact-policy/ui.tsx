...
export default function GovArtifactPolicyClient(props: { locale: string; initial: unknown; initialStatus: number; initialScopeType: "space" | "tenant" }) {
  ...
  async function load(nextScopeType?: "space" | "tenant") {
    ...
    try {
      ...
      q.set("scopeType", st);
      const res = await fetch(`${API_BASE}/governance/artifact-policy?${q.toString()}`, { headers: apiHeaders(props.locale), cache: "no-store" });
  ...
  async function save() {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/governance/artifact-policy`, {
        method: "PUT",
