...
export default function GovPolicyDebuggerClient(props: { locale: string; initial: unknown; initialStatus: number }) {
  ...
  async function refreshEpoch() {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/governance/policy/cache/epoch?${q.toString()}`, { headers: apiHeaders(props.locale), cache: "no-store" });
  ...
  async function invalidateEpoch() {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/governance/policy/cache/invalidate`, {
        method: "POST",
  ...
  async function evalPolicy() {
    ...
    try {
      const st = scopeType === "tenant" ? "tenant" : "space";
      const res = await fetch(`${API_BASE}/governance/policy/debug/evaluate`, {
        method: "POST",
        headers: { ...apiHeaders(props.locale), "content-type": "application/json" },
        cache: "no-store",
        body: JSON.stringify({
