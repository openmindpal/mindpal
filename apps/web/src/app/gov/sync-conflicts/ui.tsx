...
export default function GovSyncConflictsClient(props: { locale: string; initial: unknown; initialStatus: number }) {
  ...
  async function refresh() {
    ...
    setBusy(true);
    try {
      const res = await fetch(`${API_BASE}/sync/conflict-tickets?limit=50`, { headers: apiHeaders(props.locale), cache: "no-store" });
  ...
  async function loadTicket(ticketId: string) {
    ...
    setDetailBusy(true);
    try {
      const res = await fetch(`${API_BASE}/sync/conflict-tickets/${encodeURIComponent(ticketId)}`, { headers: apiHeaders(props.locale), cache: "no-store" });
      const json: unknown = await res.json().catch(() => null);
      if (!res.ok) throw toApiError(json);
  ...
  async function abandonTicket(ticketId: string) {
    ...
    setActionBusy(true);
    try {
      const res = await fetch(`${API_BASE}/sync/conflict-tickets/${encodeURIComponent(ticketId)}/abandon`, {
        method: "POST",
  ...
  async function resolveTicket(ticketId: string) {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/sync/conflict-tickets/${encodeURIComponent(ticketId)}/resolve`, {
        method: "POST",
        headers: { ...apiHeaders(props.locale), "content-type": "application/json" },
        cache: "no-store",
  ...
  async function applyProposal(ticketId: string) {
    ...
    setActionBusy(true);
    try {
      const res = await fetch(`${API_BASE}/sync/conflict-tickets/${encodeURIComponent(ticketId)}/apply-proposal`, {
        method: "POST",
        headers: { ...apiHeaders(props.locale), "content-type": "application/json" },
        cache: "no-store",
        body: JSON.stringify({}),
