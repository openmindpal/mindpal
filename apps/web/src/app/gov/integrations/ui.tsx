...
export default function IntegrationsClient(props: { locale: string }) {
  ...
  async function refresh() {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/governance/integrations?${q.toString()}`, { headers: apiHeaders(props.locale), cache: "no-store" });
  ...
  async function loadDetail(integrationId: string) {
    ...
    setBusy(true);
    try {
      const res = await fetch(`${API_BASE}/governance/integrations/${encodeURIComponent(integrationId)}`, { headers: apiHeaders(props.locale), cache: "no-store" });
