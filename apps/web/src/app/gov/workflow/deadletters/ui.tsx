...
export default function DeadlettersClient(props: { locale: string; initial: unknown; initialStatus: number }) {
  ...
  async function refresh() {
    ...
    const res = await fetch(`${API_BASE}/governance/workflow/deadletters?${q.toString()}`, { headers: apiHeaders(props.locale), cache: "no-store" });
  ...
  async function retry(stepId: string) {
    ...
    setBusy(stepId);
    try {
      const res = await fetch(`${API_BASE}/governance/workflow/deadletters/${encodeURIComponent(stepId)}/retry`, { method: "POST", headers: apiHeaders(props.locale) });
      const json: unknown = await res.json().catch(() => null);
  ...
  async function cancel(stepId: string) {
    ...
    setBusy(stepId);
    try {
      const res = await fetch(`${API_BASE}/governance/workflow/deadletters/${encodeURIComponent(stepId)}/cancel`, { method: "POST", headers: apiHeaders(props.locale) });
      const json: unknown = await res.json().catch(() => null);
      if (!res.ok) throw toApiError(json);
      await refresh();
    } catch (e: unknown) {
