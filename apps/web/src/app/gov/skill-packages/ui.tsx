...
export default function GovSkillPackagesClient(props: { locale: string; initial: unknown; initialStatus: number }) {
  ...
  async function refresh() {
    setError("");
    const res = await fetch(`${API_BASE}/artifacts/skill-packages?limit=50`, { headers: apiHeaders(props.locale), cache: "no-store" });
  ...
  async function upload() {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/artifacts/skill-packages/upload`, {
        method: "POST",
  ...
  async function publish() {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/tools/${encodeURIComponent(name)}/publish`, {
        method: "POST",
        headers: { ...apiHeaders(props.locale), "content-type": "application/json" },
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 12 }}>
        ...
        <Card>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            <input value={pubToolName} onChange={(e) => setPubToolName(e.currentTarget.value)} placeholder="toolName" disabled={busy} />
            <input value={pubArtifactId} onChange={(e) => setPubArtifactId(e.currentTarget.value)} placeholder="artifactId" disabled={busy} />
            <input value={pubDepsDigest} onChange={(e) => setPubDepsDigest(e.currentTarget.value)} placeholder="depsDigest (optional)" disabled={busy} />
