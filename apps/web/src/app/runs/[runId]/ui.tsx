...
export default function RunClient(props: { locale: string; runId: string; initial: unknown; initialStatus: number }) {
  ...
  const reload = useCallback(async () => {
    setError("");
    try {
      const res = await fetch(`${API_BASE}/runs/${encodeURIComponent(props.runId)}`, { headers: apiHeaders(props.locale), cache: "no-store" });
  ...
  const loadReplay = useCallback(async () => {
    ...
    setReplayBusy(true);
    try {
      const res = await fetch(`${API_BASE}/runs/${encodeURIComponent(props.runId)}/replay`, { headers: apiHeaders(props.locale), cache: "no-store" });
      const json: unknown = await res.json().catch(() => null);
      if (!res.ok) throw toApiError(json);
  ...
  const loadEvalSuites = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/governance/evals/suites?limit=20`, { headers: apiHeaders(props.locale), cache: "no-store" });
      const json: unknown = await res.json().catch(() => null);
      if (!res.ok) throw toApiError(json);
  ...
  async function cancel() {
    ...
    setBusy(true);
    try {
      const res = await fetch(`${API_BASE}/runs/${encodeURIComponent(props.runId)}/cancel`, { method: "POST", headers: apiHeaders(props.locale) });
      setHttpStatus(res.status);
      const json: unknown = await res.json().catch(() => null);
      if (!res.ok) throw toApiError(json);
      await reload();
  ...
  async function retry() {
    ...
    setBusy(true);
    try {
      const res = await fetch(`${API_BASE}/runs/${encodeURIComponent(props.runId)}/retry`, { method: "POST", headers: apiHeaders(props.locale) });
      setHttpStatus(res.status);
      const json: unknown = await res.json().catch(() => null);
      if (!res.ok) throw toApiError(json);
      await reload();
  ...
  async function reexec() {
    ...
    setBusy(true);
    try {
      const res = await fetch(`${API_BASE}/runs/${encodeURIComponent(props.runId)}/reexec`, { method: "POST", headers: apiHeaders(props.locale) });
      setHttpStatus(res.status);
      const json: unknown = await res.json().catch(() => null);
      if (!res.ok) throw toApiError(json);
  ...
  async function resolveEvidence(sourceRef: Record<string, unknown>) {
    ...
    setEvidenceBusy(key);
    try {
      const res = await fetch(`${API_BASE}/knowledge/evidence/resolve`, {
        method: "POST",
  ...
  async function addEvalCaseFromReplay(stepId: string) {
    ...
    try {
      const res = await fetch(`${API_BASE}/governance/evals/suites/${encodeURIComponent(selectedSuiteId)}/cases/from-replay`, {
        method: "POST",
        headers: { ...apiHeaders(props.locale), "content-type": "application/json" },
  ...
  async function loadCompensations(stepId: string) {
    ...
    setCompBusy(stepId);
    try {
      const res = await fetch(`${API_BASE}/governance/workflow/steps/${encodeURIComponent(stepId)}/compensations`, { headers: apiHeaders(props.locale), cache: "no-store" });
      const json: unknown = await res.json().catch(() => null);
      if (!res.ok) throw toApiError(json);
  ...
  async function compensate(stepId: string) {
    ...
    setCompBusy(stepId);
    try {
      const res = await fetch(`${API_BASE}/governance/workflow/steps/${encodeURIComponent(stepId)}/compensate`, { method: "POST", headers: apiHeaders(props.locale) });
      const json: unknown = await res.json().catch(() => null);
      if (!res.ok) throw toApiError(json);
      setCompByStepId((prev) => ({ ...prev, [stepId]: json }));
      await reload();
