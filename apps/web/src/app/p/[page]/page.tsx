...
async function loadPage(locale: string, name: string) {
  ...
  const res = await fetch(`${API_BASE}/ui/pages/${encodeURIComponent(name)}`, {
    method: "GET",
...
async function loadViewPrefs(locale: string, name: string) {
  const token = (await cookies()).get("openslin_token")?.value ?? "";
  const res = await fetch(`${API_BASE}/ui/pages/${encodeURIComponent(name)}/view-prefs`, {
    method: "GET",
    headers: apiHeaders(locale, { token }),
    cache: "no-store",
  });
...
async function loadEffectiveSchemaBy(locale: string, entity: string, schemaName: string) {
  const token = (await cookies()).get("openslin_token")?.value ?? "";
  const res = await fetch(`${API_BASE}/schemas/${encodeURIComponent(entity)}/effective?schemaName=${encodeURIComponent(schemaName)}`, {
    method: "GET",
    headers: apiHeaders(locale, { token }),
    cache: "no-store",
  });
...
async function loadEntity(locale: string, entity: string, id: string) {
  const token = (await cookies()).get("openslin_token")?.value ?? "";
  const res = await fetch(`${API_BASE}/entities/${encodeURIComponent(entity)}/${encodeURIComponent(id)}`, {
    method: "GET",
    headers: apiHeaders(locale, { token }),
    cache: "no-store",
  });
...
async function loadEntityQuery(locale: string, entity: string, body: unknown) {
  const token = (await cookies()).get("openslin_token")?.value ?? "";
  const res = await fetch(`${API_BASE}/entities/${encodeURIComponent(entity)}/query`, {
    method: "POST",
...
function formatValue(def: FieldDef | undefined, v: unknown, locale: string) {
...
export default async function PageEntry(props: {
  ...
  const name = decodeURIComponent(props.params.page);
        ...
        {variant === "cards" ? (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 12 }}>
            {items.map((r) => {
              ...
              const titleText = titleKey ? formatValue(fields[titleKey], payload[titleKey], locale) : id;
              return (
                <Card
                  key={id}
                  ...
                  <div style={{ display: "grid", gap: density === "compact" ? 4 : 8 }}>
                    {fieldKeys.slice(0, 3).map((k) => (
                      <div key={k} style={{ display: "grid", gap: 2 }}>
                        ...
                        <div>{formatValue(fields[k], payload[k], locale)}</div>
                      </div>
                    ))}
                  </div>
                </Card>
                  ...
                  <tr key={id} style={{ borderTop: "1px solid #ddd" }}>
                    ...
                    {fieldKeys.map((k) => (
                      <td key={k} style={{ padding: density === "compact" ? 4 : 6 }}>
                        {formatValue(fields[k], payload[k], locale)}
                      </td>
                    ))}
                  ...
                  {fieldKeys.map((k) => (
                    <tr key={k} style={{ borderTop: "1px solid #ddd" }}>
                      ...
                      <td>
                        {fields[k]?.type === "json" ? (
                          <details>
                            <summary>{formatValue(fields[k], entity?.payload?.[k], locale)}</summary>
                          ...
                          formatValue(fields[k], entity?.payload?.[k], locale)
                        )}
                      </td>
                    </tr>
                  ))}
