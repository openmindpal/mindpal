...
export async function executeEntityCreate(params: {
  ...
  const prior = await params.pool.query(
    `
      SELECT record_id
      FROM idempotency_records
      WHERE tenant_id = $1 AND idempotency_key = $2 AND operation = 'create' AND entity_name = $3
      LIMIT 1
    `,
    [params.tenantId, params.idempotencyKey, params.entityName],
  );
  if ((prior.rowCount ?? 0) > 0 && prior.rows[0].record_id) {
    return { recordId: prior.rows[0].record_id as string, idempotentHit: true };
  }

  const schema = await loadLatestSchema(params.pool, params.schemaName);
  if (!schema) throw new Error(`schema_not_found:${params.schemaName}`);
  validateEntityPayload(schema.schema, params.entityName, params.payload, "create");
  ...
  const insert = await params.pool.query(
    `
      ...
      RETURNING id
    `,
    [params.tenantId, params.spaceId, params.entityName, schema.name, schema.version, params.payload, params.ownerSubjectId],
  );
  const recordId = insert.rows[0].id as string;
  ...
  await params.pool.query(
    ...
    `,
    [params.tenantId, params.idempotencyKey, params.entityName, recordId],
  );
...
export async function executeEntityDelete(params: {
  ...
  const prior = await params.pool.query(
    `
      SELECT record_id
      FROM idempotency_records
      WHERE tenant_id = $1 AND idempotency_key = $2 AND operation = 'delete' AND entity_name = $3
      LIMIT 1
    `,
    [params.tenantId, params.idempotencyKey, params.entityName],
  );
  if ((prior.rowCount ?? 0) > 0 && prior.rows[0].record_id) {
    return { recordId: String(prior.rows[0].record_id), idempotentHit: true, deleted: true };
  }

  const schema = await loadLatestSchema(params.pool, params.schemaName);
  if (!schema) throw new Error(`schema_not_found:${params.schemaName}`);

  const args: any[] = [params.tenantId, params.spaceId, params.entityName, params.id];
  const where: string[] = ["tenant_id = $1", "($2::text IS NULL OR space_id = $2)", "entity_name = $3", "id = $4"];
  let idx = 4;
  const rf = compileRowFiltersWhere(
    {
      rowFilters: params.rowFilters,
      subject: { subjectId: params.ownerSubjectId, tenantId: params.tenantId, spaceId: params.spaceId },
      context: { request: { traceId: params.traceId, method: "ASYNC", path: "worker" }, resource: { type: `entity:${params.entityName}`, id: params.id } },
    },
    args,
    idx,
  );
  ...
  await params.pool.query(
    `
      ...
      ON CONFLICT (tenant_id, idempotency_key, operation, entity_name) DO NOTHING
    `,
    [params.tenantId, params.idempotencyKey, params.entityName, params.id],
  );

  return { recordId: params.id, idempotentHit: false, deleted };
}
...
export async function executeEntityUpdate(params: {
  ...
  const prior = await params.pool.query(
    `
      SELECT record_id
      FROM idempotency_records
      WHERE tenant_id = $1 AND idempotency_key = $2 AND operation = 'update' AND entity_name = $3
      LIMIT 1
    `,
    [params.tenantId, params.idempotencyKey, params.entityName],
  );
  ...
  const schema = await loadLatestSchema(params.pool, params.schemaName);
  if (!schema) throw new Error(`schema_not_found:${params.schemaName}`);
  validateEntityPayload(schema.schema, params.entityName, params.patch, "update");
  ...
  const where: string[] = ["tenant_id = $1", "($2::text IS NULL OR space_id = $2)", "entity_name = $3", "id = $4", "($6::int IS NULL OR revision = $6)"];
  let idx = 6;
  const rf = compileRowFiltersWhere(
    {
      rowFilters: params.rowFilters,
  ...
  if (res.rowCount === 0) throw new Error("conflict_or_not_found");
  const recordId = res.rows[0].id as string;
