...
async function seed() {
  ...
  await pool.query(
    `
      INSERT INTO tool_versions (tenant_id, name, version, tool_ref, status, input_schema, output_schema)
      VALUES ('tenant_dev', 'sleep', 1, 'sleep@1', 'released', $1, $2)
      ON CONFLICT (tenant_id, name, version) DO NOTHING
    `,
  ...
  await pool.query(
    `
      INSERT INTO tool_versions (tenant_id, name, version, tool_ref, status, input_schema, output_schema)
      VALUES ('tenant_dev', 'http.get', 1, 'http.get@1', 'released', $1, $2)
      ON CONFLICT (tenant_id, name, version) DO NOTHING
    `,
...
describe("workflow processor", () => {
  ...
  it("执行 entity.create@1 并写入审计", async () => {
    ...
    await processStep({ pool, jobId: job.rows[0].job_id, runId: run.rows[0].run_id, stepId: step.rows[0].step_id });
  ...
  it("timeoutMs 超时生效（timeout）", async () => {
    ...
    await expect(processStep({ pool, jobId: job.rows[0].job_id, runId: run.rows[0].run_id, stepId: step.rows[0].step_id })).rejects.toBeTruthy();
    const s = await pool.query("SELECT status, error_category FROM steps WHERE step_id = $1", [step.rows[0].step_id]);
    expect(s.rows[0].status).toBe("failed");
  ...
  it("maxConcurrency 并发限制生效（resource_exhausted）", async () => {
    ...
    const r = await Promise.allSettled([processStep({ pool, ...a }), processStep({ pool, ...b })]);
  ...
  it("rules 支持按 pathPrefix 放行/拒绝", async () => {
    ...
    try {
      ...
      await processStep({ pool, jobId: job.rows[0].job_id, runId: runOk.rows[0].run_id, stepId: stepOk.rows[0].step_id });
      ...
      await processStep({ pool, jobId: job.rows[0].job_id, runId: runBad.rows[0].run_id, stepId: stepBad.rows[0].step_id });
  ...
  it("subscription runner：poll 产出 ingress event 并推进 watermark", async () => {
    ...
    const r1 = await processSubscriptionPoll({ pool, subscriptionId });
    ...
    const r2 = await processSubscriptionPoll({ pool, subscriptionId });
  ...
  it("exchange subscription：429 尊重 retry-after 并写 backoff", async () => {
    ...
    await expect(processSubscriptionPoll({ pool, subscriptionId })).rejects.toBeTruthy();
    vi.stubGlobal("fetch", origFetch as any);
  ...
  it("webhook delivery：async queued→succeeded（含审计）", async () => {
    ...
    const ins = await pool.query(
      `
        ...
        RETURNING id
      `,
      [eventId, nonce, { channelUserId: "u1", text: "ok" }],
    );
  ...
  it("audit siem：webhook 增量投递与 cursor/outbox 生效", async () => {
    ...
    try {
      ...
      await tickAuditSiemWebhookExport({ pool, destinationsLimit: 20 });
  ...
  it("webhook delivery：失败重试→deadletter", async () => {
    ...
    const ins = await pool.query(
      `
        ...
        RETURNING id
      `,
      [eventId, nonce, { channelUserId: "u1", text: "please fail" }],
    );
    const id = ins.rows[0].id as string;
    await tickWebhookDeliveries({ pool, limit: 10 });
