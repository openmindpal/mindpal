...
registerTicker({
  ...
  intervalMs: () => Math.max(Number(process.env.REGRESSION_EVAL_INTERVAL_MS) || 5 * 60_000, 30_000),
  lockKey: "tick:regression-eval",
  lockTtlMs: () => Math.max(Number(process.env.REGRESSION_EVAL_INTERVAL_MS) || 5 * 60_000, 30_000),
...
registerTicker({
  name: "memory-purge",
  intervalMs: () => Math.max(Number(process.env.MEMORY_LIFECYCLE_INTERVAL_MS) || 60 * 60_000, 60_000),
  lockKey: "tick:memory-purge",
  lockTtlMs: () => Math.max(Number(process.env.MEMORY_LIFECYCLE_INTERVAL_MS) || 60 * 60_000, 60_000),
...
registerTicker({
  name: "memory-compact",
  intervalMs: () => Math.max(Number(process.env.MEMORY_COMPACT_INTERVAL_MS) || 4 * 60 * 60_000, 60_000),
  lockKey: "tick:memory-compact",
  lockTtlMs: () => Math.max(Number(process.env.MEMORY_COMPACT_INTERVAL_MS) || 4 * 60 * 60_000, 60_000),
  handler: async ({ pool, queue }) => {
    const tsRes = await pool.query(
      `SELECT DISTINCT tenant_id, space_id FROM memory_entries WHERE deleted_at IS NULL LIMIT 200`,
    );
    for (const row of tsRes.rows as any[]) {
      ...
      await compactSimilarMemories({ pool, tenantId, spaceId }).catch((err) =>
        console.error(`memory compact failed tenant=${tenantId} space=${spaceId}`, err),
      );
      const distillResult = await distillSessionSummaries({
        ...
        console.error(`memory distill failed tenant=${tenantId} space=${spaceId}`, err);
        return null;
      });

      const upgradeResult = await distillUpgradeMemories({ pool, tenantId, spaceId }).catch((err) => {
        console.error(`memory distill_upgrade failed tenant=${tenantId} space=${spaceId}`, err);
        return null;
      });
      ...
      if (allDistilledIds.length > 0) {
        ...
        ).catch((err) => console.error(`memory.embed enqueue failed tenant=${tenantId}`, err));
      }
    }
  },
});
...
registerTicker({
  name: "memory-decay",
  intervalMs: () => Math.max(Number(process.env.MEMORY_DECAY_INTERVAL_MS) || 60 * 60_000, 60_000),
  lockKey: "tick:memory-decay",
  lockTtlMs: () => Math.max(Number(process.env.MEMORY_DECAY_INTERVAL_MS) || 60 * 60_000, 60_000),
...
registerTicker({
  ...
  intervalMs: () => Math.max(Number(process.env.MEMORY_EMBEDDING_BACKFILL_INTERVAL_MS) || 2 * 60 * 60_000, 60_000),
  lockKey: "tick:memory-embed-backfill",
  lockTtlMs: () => Math.max(Number(process.env.MEMORY_EMBEDDING_BACKFILL_INTERVAL_MS) || 2 * 60 * 60_000, 60_000),
...
registerTicker({
  name: "model-probe",
  intervalMs: () => Math.max(Number(process.env.MODEL_PROBE_INTERVAL_MS) || 30 * 60_000, 60_000),
  lockKey: "tick:model-probe",
  lockTtlMs: () => Math.max(Number(process.env.MODEL_PROBE_INTERVAL_MS) || 30 * 60_000, 60_000),
  handler: async ({ pool }) => {
    ...
    for (const t of tenants.rows) {
      ...
      await runModelProbing({ pool, tenantId: tid }).catch((e: any) =>
        console.error(`model probing failed for tenant ${tid}`, e),
      );
    }
  },
});
...
registerTicker({
  ...
  intervalMs: () => Math.max(Number(process.env.APPROVAL_EXPIRY_INTERVAL_MS) || 60_000, 10_000),
  lockKey: "tick:approval-expiry",
  lockTtlMs: () => Math.max(Number(process.env.APPROVAL_EXPIRY_INTERVAL_MS) || 60_000, 10_000),
...
registerTicker({
  ...
  intervalMs: () => Math.max(Number(process.env.NOTIFICATION_QUEUE_INTERVAL_MS) || 3_000, 1_000),
  lockKey: "tick:notification-queue",
  lockTtlMs: () => Math.max(Number(process.env.NOTIFICATION_QUEUE_INTERVAL_MS) || 3_000, 1_000) + 2_000,
