...
export async function tickCronTriggers(params: { pool: Pool; queue: Queue }) {
  ...
  for (const row of candidates.rows as any[]) {
    ...
    if (!trigger) continue;
    const t: TriggerDefinitionRow = {
      triggerId: String(trigger.trigger_id),
      tenantId: String(trigger.tenant_id),
      spaceId: trigger.space_id ? String(trigger.space_id) : null,
      type: trigger.type,
      status: trigger.status,
      cronExpr: trigger.cron_expr ?? null,
      cronTz: trigger.cron_tz ?? null,
      cronMisfirePolicy: String(trigger.cron_misfire_policy ?? "skip"),
      nextFireAt: trigger.next_fire_at ?? null,
      eventSource: trigger.event_source ?? null,
      eventFilter: trigger.event_filter_json ?? null,
      eventWatermark: trigger.event_watermark_json ?? null,
      targetKind: trigger.target_kind,
      targetRef: String(trigger.target_ref),
      inputMapping: trigger.input_mapping_json ?? null,
      idempotencyKeyTemplate: trigger.idempotency_key_template ?? null,
      idempotencyWindowSec: Number(trigger.idempotency_window_sec ?? 3600),
      rateLimitPerMin: Number(trigger.rate_limit_per_min ?? 60),
      lastRunAt: trigger.last_run_at ?? null,
      createdBySubjectId: trigger.created_by_subject_id ?? null,
    };
