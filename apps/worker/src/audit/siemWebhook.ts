...
async function loadWebhookConfig(params: { pool: Pool; tenantId: string; secretId: string }) {
  const res = await params.pool.query(
    `
      SELECT
        s.id,
...
async function deliverBatch(params: { pool: Pool; dest: { id: string; tenantId: string; secretId: string; batchSize: number; timeoutMs: number } }) {
  ...
  try {
    const cfg = await loadWebhookConfig({ pool: params.pool, tenantId: params.dest.tenantId, secretId: params.dest.secretId });
...
export async function tickAuditSiemWebhookExport(params: { pool: Pool; destinationsLimit?: number }) {
  ...
  for (const d of dests) {
    ...
    for (let i = 0; i < 10; i++) {
      const out = await deliverBatch({ pool: params.pool, dest: { id: d.id, tenantId: d.tenantId, secretId: d.secretId, batchSize: d.batchSize, timeoutMs: d.timeoutMs } });
      if (!out.sent) break;
    }
  }
  return { ok: true };
