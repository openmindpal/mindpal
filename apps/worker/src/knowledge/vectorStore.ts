...
export type VectorStoreChunkEmbedding = {
  chunkId: string;
  documentId: string;
...
export type VectorStore = {
  ...
  upsertEmbeddings(params: { pool: Pool; embeddings: VectorStoreChunkEmbedding[] }): Promise<{ ok: true } | { ok: false; degraded: boolean; reason: string }>;
  deleteEmbeddings(params: { pool: Pool; chunkIds: string[] }): Promise<{ ok: true } | { ok: false; degraded: boolean; reason: string }>;
...
class ExternalVectorStore implements VectorStore {
  ...
  async upsertEmbeddings(params: { pool: Pool; embeddings: VectorStoreChunkEmbedding[] }) {
    const url = new URL(this.cfg.endpoint);
    url.pathname = url.pathname.replace(/\/$/, "") + "/v1/upsert";
    ...
    try {
      const res = await fetch(url.toString(), { method: "POST", headers, body: JSON.stringify({ format: "vectorStore.upsert.v1", embeddings: params.embeddings }), signal: controller.signal } as any);
      if (!res.ok) return { ok: false as const, degraded: true, reason: `vector_store_http_${res.status}` };
      return { ok: true as const };
    } catch (e: any) {
      return { ok: false as const, degraded: true, reason: String(e?.message ?? e ?? "vector_store_error") };
    } finally {
      clearTimeout(t);
    }
  }
  async deleteEmbeddings(params: { pool: Pool; chunkIds: string[] }) {
    const url = new URL(this.cfg.endpoint);
    url.pathname = url.pathname.replace(/\/$/, "") + "/v1/delete";
    const headers: Record<string, string> = { "content-type": "application/json" };
    if (this.cfg.bearerToken) headers.authorization = `Bearer ${this.cfg.bearerToken}`;
    const controller = new AbortController();
    const t = setTimeout(() => controller.abort(), this.cfg.timeoutMs);
    try {
      const res = await fetch(url.toString(), { method: "POST", headers, body: JSON.stringify({ format: "vectorStore.delete.v1", chunkIds: params.chunkIds }), signal: controller.signal } as any);
      if (!res.ok) return { ok: false as const, degraded: true, reason: `vector_store_http_${res.status}` };
      return { ok: true as const };
    } catch (e: any) {
      return { ok: false as const, degraded: true, reason: String(e?.message ?? e ?? "vector_store_error") };
