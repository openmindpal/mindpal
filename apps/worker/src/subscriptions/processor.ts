...
async function runImapPoll(params: { pool: Pool; sub: any; runId: string; traceId: string; watermarkBefore: any }) {
  ...
  const summary = {
    ...
    subjectDigest: { len: 0 },
    internalDate,
    messageIdDigest: { len: 0 },
  };
  const nonce = `uid:${uid}`;

  const bodyBytes = Buffer.from(`mvp imap body uid=${uid}`, "utf8");
  const bodySha = `sha256:${sha256HexBytes(bodyBytes)}`;
  const bodyMeta = { contentType: "text/plain", byteSize: bodyBytes.length, sha256: bodySha };

  const attachmentBytes = cfg.mailbox.toUpperCase().includes("OVERSIZE")
    ? Buffer.alloc(6 * 1024 * 1024, 0x61)
    : Buffer.from(`mvp imap attachment uid=${uid}`, "utf8");
  const attSha = `sha256:${sha256HexBytes(attachmentBytes)}`;
  const attMeta = { fileName: `attachment-${uid}.txt`, contentType: "text/plain", byteSize: attachmentBytes.length, sha256: attSha };
  ...
  const inserted = await params.pool.query(
    `
      ...
      RETURNING id
    `,
    [cfg.tenantId, workspaceId, eventId, nonce, initialDigest, JSON.stringify(initialPayload), params.runId, params.traceId, spaceId],
  );
  ...
  const totalSize = bodyBytes.length + attachmentBytes.length;
  const allowBody = bodyBytes.length <= singleMax && totalSize <= totalMax;
  const allowAttachment = attachmentBytes.length <= singleMax && totalSize <= totalMax;
  ...
  if (allowBody) {
    const bRes = await params.pool.query(
      ...
      [
        ...
        bodyMeta.byteSize,
        bodySha,
        { provider: "imap", workspaceId, eventId, kind: "body" },
        null,
        null,
        bodyBytes,
        null,
  ...
  if (allowAttachment) {
    const aRes = await params.pool.query(
      ...
      [
        ...
        attMeta.byteSize,
        attSha,
        { provider: "imap", workspaceId, eventId, kind: "attachment", fileName: attMeta.fileName },
        null,
        null,
        attachmentBytes,
        null,
  ...
  const finalPayload = {
    ...initialPayload,
    body: allowBody ? { ...bodyMeta, mediaRef: created.body } : { ...bodyMeta },
    attachments: [
      allowAttachment ? { ...attMeta, mediaRef: (created.attachments ?? [])[0] } : { ...attMeta },
    ],
...
async function runExchangePoll(params: { pool: Pool; sub: any; runId: string; traceId: string; watermarkBefore: any }) {
  const connectorInstanceId = params.sub.connector_instance_id as string | null;
  if (!connectorInstanceId) throw new Error("connector_instance_missing");
...
export async function processSubscriptionPoll(params: { pool: Pool; subscriptionId: string }) {
    ...
    const r =
      sub.provider === "imap"
        ...
        : sub.provider === "exchange"
          ? await runExchangePoll({ pool: params.pool, sub, runId, traceId, watermarkBefore })
