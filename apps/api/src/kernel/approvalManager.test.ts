/**
 * approvalManager.ts + approvalRuleEngine.ts 单元测试
 *
 * 所有审批规则均来自数据库（approval_rules 表），
 * 测试通过 mock pool.query 模拟规则加载，验证规则匹配逻辑。
 */
import { describe, it, expect, vi } from "vitest";
import { DEFAULT_APPROVAL_POLICY } from "./approvalManager";
...
/* ================================================================== */
/*  DEFAULT_APPROVAL_POLICY 常量验证                                    */
/* ================================================================== */

describe("DEFAULT_APPROVAL_POLICY", () => {
  it("默认过期时间为 1440 分钟（24小时）", () => {
    expect(DEFAULT_APPROVAL_POLICY.expirationMinutes).toBe(1440);
  });

  it("默认升级时间为 240 分钟（4小时）", () => {
    expect(DEFAULT_APPROVAL_POLICY.escalationMinutes).toBe(240);
  });

  it("默认无升级目标", () => {
    expect(DEFAULT_APPROVAL_POLICY.escalationTarget).toBeNull();
  });

  it("默认允许批量审批", () => {
    expect(DEFAULT_APPROVAL_POLICY.allowBatchApproval).toBe(true);
  });

  it("默认不要求双人审批", () => {
    expect(DEFAULT_APPROVAL_POLICY.requireDualApproval).toBe(false);
  });

  it("默认过期自动拒绝", () => {
    expect(DEFAULT_APPROVAL_POLICY.autoRejectOnExpiry).toBe(true);
  });
});
