...
/* ================================================================== */
/*  4. Anchor Pattern Rules — 意图锚定规则加载                              */
/* ================================================================== */

const _anchorLogger = new StructuredLogger({ module: "intentAnchoring.rules" });
 ...
 * 回退到内置默认规则。
 *
 * 优先使用 intentVocabulary.getActiveVocab() 动态词表，
 * 本函数仅侜更底层的配置文件回退。
 */
export function loadAnchorRules(): {
  ...
  try {
    ...
    _anchorLogger.warn(`Failed to load anchor rules: ${(err as Error)?.message}`);
  }
  return {
