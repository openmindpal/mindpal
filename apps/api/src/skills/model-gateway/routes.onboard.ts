...
export const modelOnboardRoutes: FastifyPluginAsync = async (app) => {
  app.post("/models/onboard", async (req) => {
    ...
    const body = z
      .object({
        provider: z.enum(["openai_compatible", "deepseek", "hunyuan", "qwen", "doubao", "zhipu", "kimi", "ernie", "spark", "baichuan", "yi", "minimax", "step", "sensenova", "custom_openai", "anthropic", "custom_anthropic", "gemini", "custom_gemini"]),
