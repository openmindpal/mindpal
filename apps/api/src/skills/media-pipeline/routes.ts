...
import { createMediaJob } from "./modules/jobRepo";
import { createMediaObject, getMediaContent, getMediaObject } from "./modules/mediaRepo";
  ...
  app.post("/media/objects", async (req) => {
    const body = z
      .object({
        ...
        traceability: z.unknown().optional(),
      })
      .parse(req.body);
