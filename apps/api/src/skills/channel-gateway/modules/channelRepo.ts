...
export async function listOutboxMessages(params: {
  pool: Pool;
  tenantId: string;
  status?: string | null;
  ...
  if (params.workspaceId) {
    args.push(params.workspaceId);
    where.push(`workspace_id = $${args.length}`);
  }
  if (params.channelChatId) {
    args.push(params.channelChatId);
    where.push(`channel_chat_id = $${args.length}`);
  }
  args.push(params.limit);
  const res = await params.pool.query(
    `
...
export async function markOutboxQueued(params: { pool: Pool; tenantId: string; id: string }) {
  const res = await params.pool.query(
    `
      UPDATE channel_outbox_messages
      SET status = 'queued', next_attempt_at = NULL, last_error_category = NULL, last_error_digest = NULL, deadlettered_at = NULL, updated_at = now()
...
export async function markOutboxCanceled(params: { pool: Pool; tenantId: string; id: string }) {
  const res = await params.pool.query(
    `
      UPDATE channel_outbox_messages
...
export async function markOutboxDelivered(params: { pool: Pool; tenantId: string; ids: string[] }) {
  ...
  const res = await params.pool.query(
    `
      UPDATE channel_outbox_messages
      SET status = CASE
            WHEN status IN ('queued','failed','processing') THEN 'delivered'
            ELSE status
          END,
...
export async function markOutboxAcked(params: { pool: Pool; tenantId: string; ids: string[] }) {
  ...
  const res = await params.pool.query(
    `
      UPDATE channel_outbox_messages
      SET status = CASE
            WHEN status IN ('delivered','queued','failed','processing') THEN 'acked'
            ELSE status
          END,
