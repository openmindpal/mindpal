...
import { resetTenantRateLimit } from "../modules/modelGateway/rateLimit";
...
describe.sequential("e2e", { timeout: 60_000 }, () => {
  ...
  it("governance：schema.publish 新增必填字段需要 migration gate", async () => {
    ...
    expect((release1.json() as any).errorCode).toBe("MIGRATION_REQUIRED");
  ...
  it("orchestrator：可生成工具建议与 UI 指令并写审计", async () => {
    ...
    const res = await app.inject({
      ...
      payload: JSON.stringify({ message: "打开笔记并新建一条" }),
    });
    expect(res.statusCode).toBe(200);
    const body = res.json() as any;
    expect(body.turnId).toBeTruthy();
    expect(Array.isArray(body.toolSuggestions)).toBe(true);
    expect(body.toolSuggestions.some((s: any) => String(s.toolRef).startsWith("entity.create@"))).toBe(true);
    expect(body.toolSuggestions.some((s: any) => Boolean(s.suggestionId))).toBe(true);
    expect(body.toolSuggestions.some((s: any) => Boolean(s.idempotencyKey))).toBe(true);
    expect(body.uiDirective?.openView).toBe("page");
    expect(body.uiDirective?.viewParams?.name).toBe("notes.list");
  ...
  it("orchestrator：closed-loop 写入 taskState（plan/evidenceRefs/guard/execution）", async () => {
    ...
    const res = await app.inject({
      ...
      payload: JSON.stringify({ goal: "查找与笔记相关的证据", purpose: "test", retriever: { query: "notes", limit: 3 } }),
    });
    expect(res.statusCode).toBe(200);
    const body = res.json() as any;
    expect(String(body.runId ?? "")).toMatch(/[0-9a-f-]{36}/i);
    expect(body.plan?.version).toBe("v3");
    ...
    expect(tBody.taskState?.plan?.version).toBe("v3");
  ...
  it("orchestrator：execute 支持 queued/needs_approval，且校验 toolRef 与 input", async () => {
    ...
    const publishEntityCreate = await app.inject({
      method: "POST",
      url: "/tools/entity.create/publish",
      headers: { authorization: "Bearer admin", "x-tenant-id": "tenant_dev", "x-space-id": "space_dev", "x-trace-id": "t-orch-exec-pub-entity-create", "content-type": "application/json" },
      payload: JSON.stringify({
        scope: "write",
        resourceType: "entity",
        action: "create",
        ...
        outputSchema: { fields: { id: { type: "string", required: false } } },
      }),
    });
    ...
    const enableEntityCreate = await app.inject({
      ...
      headers: { authorization: "Bearer admin", "x-trace-id": "t-orch-exec-enable-entity-create", "content-type": "application/json" },
      payload: JSON.stringify({ scope: "space" }),
    });
  ...
  it("model gateway：catalog/binding/invoke/allowedDomains/限流", async () => {
    if (!canRun) return;
    await resetTenantRateLimit(app.redis);
    ...
    expect(fallbackAttempts.some((a) => a.errorCode === "PROVIDER_NOT_IMPLEMENTED" && a.reason)).toBe(true);

    const budgetPurpose = `budget-${crypto.randomUUID()}`;
    const budget = await app.inject({
      method: "PUT",
      url: "/governance/model-gateway/budgets",
      headers: {
        authorization: "Bearer admin",
        "x-tenant-id": "tenant_dev",
        "x-space-id": "space_dev",
        "x-trace-id": "t-model-budget-set",
        "content-type": "application/json",
      },
      payload: JSON.stringify({ scope: "space", purpose: budgetPurpose, hardDailyTokens: 1 }),
    });
    expect(budget.statusCode).toBe(200);

    const invokeBudget1 = await app.inject({
      method: "POST",
      url: "/models/chat",
      headers: {
        authorization: "Bearer admin",
        "x-tenant-id": "tenant_dev",
        "x-space-id": "space_dev",
        "x-trace-id": "t-model-budget-1",
        "content-type": "application/json",
      },
      payload: JSON.stringify({ purpose: budgetPurpose, modelRef: "mock:echo-1", messages: [{ role: "user", content: "hi" }] }),
    });
    expect(invokeBudget1.statusCode).toBe(200);

    const invokeBudget2 = await app.inject({
      method: "POST",
      url: "/models/chat",
      headers: {
        authorization: "Bearer admin",
        "x-tenant-id": "tenant_dev",
        "x-space-id": "space_dev",
        "x-trace-id": "t-model-budget-2",
        "content-type": "application/json",
      },
      payload: JSON.stringify({ purpose: budgetPurpose, modelRef: "mock:echo-1", messages: [{ role: "user", content: "hi2" }] }),
    });
    expect(invokeBudget2.statusCode).toBe(403);
    expect((invokeBudget2.json() as any).errorCode).toBe("MODEL_BUDGET_EXCEEDED");
    ...
    await resetTenantRateLimit(app.redis);
    process.env.MODEL_RPM = "100";
    process.env.MODEL_USER_RPM = "2";
    process.env.MODEL_SCENE_RPM = "1";
    const invokeScene1 = await app.inject({
      method: "POST",
      url: "/models/chat",
      headers: {
        authorization: "Bearer admin",
        "x-tenant-id": "tenant_dev",
        "x-space-id": "space_dev",
        "x-trace-id": "t-model-scene-1",
        "content-type": "application/json",
      },
      payload: JSON.stringify({ purpose: "test", scene: "scene-a", modelRef: "mock:echo-1", messages: [{ role: "user", content: "scene-1" }] }),
    });
    expect(invokeScene1.statusCode).toBe(200);
    const invokeScene2 = await app.inject({
      method: "POST",
      url: "/models/chat",
      headers: {
        authorization: "Bearer admin",
        "x-tenant-id": "tenant_dev",
        "x-space-id": "space_dev",
        "x-trace-id": "t-model-scene-2",
        "content-type": "application/json",
      },
      payload: JSON.stringify({ purpose: "test", scene: "scene-a", modelRef: "mock:echo-1", messages: [{ role: "user", content: "scene-2" }] }),
    });
    expect(invokeScene2.statusCode).toBe(429);
    expect((invokeScene2.json() as any).dimension).toBe("scene");

    const invokeUser = await app.inject({
      method: "POST",
      url: "/models/chat",
      headers: {
        authorization: "Bearer admin",
        "x-tenant-id": "tenant_dev",
        "x-space-id": "space_dev",
        "x-trace-id": "t-model-user-limit",
        "content-type": "application/json",
      },
      payload: JSON.stringify({ purpose: "test", scene: "scene-b", modelRef: "mock:echo-1", messages: [{ role: "user", content: "user-limit" }] }),
    });
    expect(invokeUser.statusCode).toBe(429);
    expect((invokeUser.json() as any).dimension).toBe("user");
    process.env.MODEL_USER_RPM = "0";
    process.env.MODEL_SCENE_RPM = "0";

    await resetTenantRateLimit(app.redis);
    process.env.MODEL_RPM = "2";
    const invoke2 = await app.inject({
      ...
      headers: {
        ...
        "x-tenant-id": "tenant_dev",
        "x-space-id": "space_dev",
        "x-trace-id": "t-model-invoke-2",
        "content-type": "application/json",
      },
      payload: JSON.stringify({ purpose: "test", modelRef: "mock:echo-1", messages: [{ role: "user", content: "hi2" }] }),
    });
    expect(invoke2.statusCode).toBe(200);
    const invoke3 = await app.inject({
      method: "POST",
      url: "/models/chat",
      headers: {
        authorization: "Bearer admin",
        "x-tenant-id": "tenant_dev",
        "x-space-id": "space_dev",
        "x-trace-id": "t-model-invoke-3",
        "content-type": "application/json",
      },
      payload: JSON.stringify({ purpose: "test", modelRef: "mock:echo-1", messages: [{ role: "user", content: "hi3" }] }),
    });
    expect(invoke3.statusCode).toBe(200);
    const invoke4 = await app.inject({
      method: "POST",
      url: "/models/chat",
      headers: {
        authorization: "Bearer admin",
        "x-tenant-id": "tenant_dev",
        "x-space-id": "space_dev",
        "x-trace-id": "t-model-invoke-4",
        "content-type": "application/json",
      },
      payload: JSON.stringify({ purpose: "test", modelRef: "mock:echo-1", messages: [{ role: "user", content: "hi4" }] }),
    });
    expect(invoke4.statusCode).toBe(429);
    const b4 = invoke4.json() as any;
    expect(b4.errorCode).toBe("RATE_LIMITED");
  ...
  it("model gateway：openai provider（成功/失败/超时）不泄露 secret", async () => {
    ...
    const bind = await app.inject({
      ...
      payload: JSON.stringify({ modelRef: "openai:gpt-4o-mini", baseUrl: "https://api.openai.com", connectorInstanceId: instId, secretId }),
    });
    expect(bind.statusCode).toBe(200);
    ...
    expect(String(calls[0].url)).toContain("/v1/chat/completions");
  ...
  it("model gateway：onboard（保存 baseUrl/幂等/回滚）", async () => {
    ...
    expect(String(b1.baseUrl)).toBe("https://example.com");
  ...
  it.sequential("model gateway: bindings can include multiple secrets", async () => {
    ...
    const bind = await app.inject({
      ...
      payload: JSON.stringify({ modelRef: "openai:gpt-4o-mini", baseUrl: "https://api.openai.com", connectorInstanceId: instId, secretIds: [s1Id, s2Id] }),
    });
    expect(bind.statusCode).toBe(200);
  ...
  it("model gateway：constraints candidates 优先生效且熔断会跳过候选", async () => {
    ...
    const openaiBind = await app.inject({
      ...
      payload: JSON.stringify({ modelRef: "openai:gpt-4o-mini", baseUrl: "https://api.openai.com", connectorInstanceId: openaiInstId, secretId: openaiSecretId }),
    });
  ...
  it("governance：models usage 支持按范围聚合查询", async () => {
    if (!canRun) return;
    const res = await app.inject({
      method: "GET",
      url: "/governance/models/usage?range=24h",
      headers: { authorization: "Bearer admin", "x-trace-id": "t-model-usage" },
    });
    expect(res.statusCode).toBe(200);
    const body = res.json() as any;
  ...
  it("governance：路由策略、RPM 配置与工具并发生效", async () => {
    if (!canRun) return;
    await resetTenantRateLimit(app.redis);
    process.env.MODEL_RPM = "9999";

    const mockInstName = `policy-mock-${crypto.randomUUID()}`;
    const mockInst = await app.inject({
      method: "POST",
      url: "/connectors/instances",
      headers: { authorization: "Bearer admin", "x-tenant-id": "tenant_dev", "x-space-id": "space_dev", "x-trace-id": "t-policy-mock-conn", "content-type": "application/json" },
      payload: JSON.stringify({ name: mockInstName, typeName: "generic.api_key", egressPolicy: { allowedDomains: ["mock.local"] } }),
    });
    expect(mockInst.statusCode).toBe(200);
    const mockInstId = (mockInst.json() as any).instance?.id as string;
    ...
    const mockSecret = await app.inject({
      method: "POST",
      url: "/secrets",
      headers: { authorization: "Bearer admin", "x-tenant-id": "tenant_dev", "x-space-id": "space_dev", "x-trace-id": "t-policy-mock-secret", "content-type": "application/json" },
      payload: JSON.stringify({ connectorInstanceId: mockInstId, payload: { apiKey: "sk_test_xxx" } }),
    });
    expect(mockSecret.statusCode).toBe(200);
    const mockSecretId = (mockSecret.json() as any).secret?.id as string;
    ...
    const mockBind = await app.inject({
      method: "POST",
      url: "/models/bindings",
      headers: { authorization: "Bearer admin", "x-tenant-id": "tenant_dev", "x-space-id": "space_dev", "x-trace-id": "t-policy-mock-bind", "content-type": "application/json" },
      payload: JSON.stringify({ modelRef: "mock:echo-1", baseUrl: "http://mock.local", connectorInstanceId: mockInstId, secretId: mockSecretId }),
    });
    expect(mockBind.statusCode).toBe(200);

    const policyUpsert = await app.inject({
      method: "PUT",
      url: "/governance/model-gateway/routing/test",
      headers: { authorization: "Bearer admin", "x-trace-id": "t-policy-upsert", "content-type": "application/json" },
      payload: JSON.stringify({ primaryModelRef: "mock:echo-1", fallbackModelRefs: [], enabled: true }),
    });
    expect(policyUpsert.statusCode).toBe(200);

    const invokePolicyWins = await app.inject({
      method: "POST",
      url: "/models/chat",
      headers: { authorization: "Bearer admin", "x-tenant-id": "tenant_dev", "x-space-id": "space_dev", "x-trace-id": "t-policy-invoke", "content-type": "application/json" },
      payload: JSON.stringify({ purpose: "test", modelRef: "openai:gpt-4o-mini", messages: [{ role: "user", content: "hi" }] }),
    });
    expect(invokePolicyWins.statusCode).toBe(200);
    expect((invokePolicyWins.json() as any).outputText).toBe("echo:hi");

    const openaiInstName = `policy-openai-${crypto.randomUUID()}`;
    const openaiInst = await app.inject({
      method: "POST",
      url: "/connectors/instances",
      headers: { authorization: "Bearer admin", "x-tenant-id": "tenant_dev", "x-space-id": "space_dev", "x-trace-id": "t-policy-openai-conn", "content-type": "application/json" },
      payload: JSON.stringify({ name: openaiInstName, typeName: "model.openai", egressPolicy: { allowedDomains: ["api.openai.com"] } }),
    });
    expect(openaiInst.statusCode).toBe(200);
    const openaiInstId = (openaiInst.json() as any).instance?.id as string;
    const openaiSecret = await app.inject({
      method: "POST",
      url: "/secrets",
      headers: { authorization: "Bearer admin", "x-tenant-id": "tenant_dev", "x-space-id": "space_dev", "x-trace-id": "t-policy-openai-secret", "content-type": "application/json" },
      payload: JSON.stringify({ connectorInstanceId: openaiInstId, payload: { apiKey: "sk_test_openai_xxx" } }),
    });
    expect(openaiSecret.statusCode).toBe(200);
    const openaiSecretId = (openaiSecret.json() as any).secret?.id as string;
    const openaiBind = await app.inject({
      method: "POST",
      url: "/models/bindings",
      headers: { authorization: "Bearer admin", "x-tenant-id": "tenant_dev", "x-space-id": "space_dev", "x-trace-id": "t-policy-openai-bind", "content-type": "application/json" },
      payload: JSON.stringify({ modelRef: "openai:gpt-4o-mini", baseUrl: "https://api.openai.com", connectorInstanceId: openaiInstId, secretId: openaiSecretId }),
    });
    expect(openaiBind.statusCode).toBe(200);

    const policyFallback = await app.inject({
      method: "PUT",
      url: "/governance/model-gateway/routing/fallback",
      headers: { authorization: "Bearer admin", "x-trace-id": "t-fallback-upsert", "content-type": "application/json" },
      payload: JSON.stringify({ primaryModelRef: "openai:gpt-4o-mini", fallbackModelRefs: ["mock:echo-1"], enabled: true }),
    });
    expect(policyFallback.statusCode).toBe(200);

    const origFetch = (globalThis as any).fetch;
    vi.stubGlobal("fetch", async () => ({ ok: false, status: 500, json: async () => ({}) }) as any);
    let cursor = "0";
    do {
      const [next, keys] = (await app.redis.scan(cursor, "MATCH", "cb:model_chat:*:openai:gpt-4o-mini", "COUNT", "200")) as unknown as [string, string[]];
      cursor = next;
    ...
    const invokeFallback = await app.inject({
      method: "POST",
      url: "/models/chat",
      headers: { authorization: "Bearer admin", "x-tenant-id": "tenant_dev", "x-space-id": "space_dev", "x-trace-id": "t-fallback-invoke", "content-type": "application/json" },
      payload: JSON.stringify({ purpose: "fallback", messages: [{ role: "user", content: "hi-fallback" }] }),
    });
    vi.stubGlobal("fetch", origFetch);
    expect(invokeFallback.statusCode).toBe(200);
    const fb = invokeFallback.json() as any;
    expect(fb.outputText).toBe("echo:hi-fallback");
    expect(fb.routingDecision?.provider).toBe("mock");
    const auditFb = await pool.query("SELECT output_digest FROM audit_events WHERE trace_id = $1 LIMIT 1", ["t-fallback-invoke"]);
    expect(auditFb.rowCount).toBe(1);
    expect(JSON.stringify(auditFb.rows[0].output_digest ?? {})).toMatch(/MODEL_UPSTREAM_FAILED|CIRCUIT_OPEN/);

    const rpmUpd = await app.inject({
      method: "PUT",
      url: "/governance/model-gateway/limits",
      headers: { authorization: "Bearer admin", "x-trace-id": "t-rpm-upd", "content-type": "application/json" },
      payload: JSON.stringify({ scope: "space", modelChatRpm: 1 }),
    });
    expect(rpmUpd.statusCode).toBe(200);

    await resetTenantRateLimit(app.redis);
    const rpm1 = await app.inject({
      method: "POST",
      url: "/models/chat",
      headers: { authorization: "Bearer admin", "x-tenant-id": "tenant_dev", "x-space-id": "space_dev", "x-trace-id": "t-rpm-1", "content-type": "application/json" },
      payload: JSON.stringify({ purpose: "rpm", modelRef: "mock:echo-1", messages: [{ role: "user", content: "hi1" }] }),
    });
    expect(rpm1.statusCode).toBe(200);
    const rpm2 = await app.inject({
      method: "POST",
      url: "/models/chat",
      headers: { authorization: "Bearer admin", "x-tenant-id": "tenant_dev", "x-space-id": "space_dev", "x-trace-id": "t-rpm-2", "content-type": "application/json" },
      payload: JSON.stringify({ purpose: "rpm", modelRef: "mock:echo-1", messages: [{ role: "user", content: "hi2" }] }),
    });
    expect(rpm2.statusCode).toBe(429);
    expect((rpm2.json() as any).errorCode).toBe("RATE_LIMITED");
  ...
  it("collab runtime：创建→执行→事件流可查询", async () => {
    ...
    expect(["queued", "needs_approval"].includes(String(b.status))).toBe(true);

    if (String(b.status) === "queued") {
      await processStep({ pool, jobId: String(b.jobId), runId: String(b.runId), stepId: String(b.stepId) });
    }
  ...
  it("collab runtime：toolPolicy 拒绝会返回 409 且写入事件", async () => {
    ...
    const create = await app.inject({
      ...
      payload: JSON.stringify({
        message: "帮我搜索 hello",
        roles: [{ roleName: "planner", mode: "auto", toolPolicy: { allowedTools: [] } }, { roleName: "executor", mode: "auto" }, { roleName: "reviewer", mode: "auto" }],
      }),
    });
