...
// ── digest 工具函数（原 digests.ts，<30 行已内联） ─────────────
...
async function tryWriteAuditEvent(app: FastifyInstance, params: { req: FastifyRequest; reply: FastifyReply; result: "success" | "denied" | "error"; latencyMs?: number }) {
  ...
  try {
    ...
    try {
      const r = await enqueueReadAuditOutboxFallback(app, { req: params.req, result: params.result });
...
export async function finalizeAudit(app: FastifyInstance, params: { req: FastifyRequest; reply: FastifyReply; payload: unknown; mergeDigest: boolean }): Promise<unknown> {
  ...
  const auditWriteOut = await tryWriteAuditEvent(app, { req: params.req, reply: params.reply, result, latencyMs });
  if (auditWriteOut) {
