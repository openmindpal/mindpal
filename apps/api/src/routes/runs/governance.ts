/**
 * runs/governance.ts — 治理/Deadletter/补偿 路由
 */
...
export const runsGovernanceRoutes: FastifyPluginAsync = async (app) => {
  app.get("/governance/workflow/deadletters", async (req) => {
    ...
    const decision = await requirePermission({ req, ...PERM.GOVERNANCE_WORKFLOW_DEADLETTER_READ });
  ...
  app.get("/governance/workflow/steps/:stepId/output/reveal", async (req) => {
    ...
    const decision = await requirePermission({ req, ...PERM.GOVERNANCE_WORKFLOW_STEP_OUTPUT_REVEAL });
    req.ctx.audit!.policyDecision = decision;
  ...
  app.post("/governance/workflow/steps/:stepId/compensate", async (req) => {
    ...
    const decision = await requirePermission({ req, ...PERM.GOVERNANCE_WORKFLOW_STEP_COMPENSATE });
    req.ctx.audit!.policyDecision = decision;
    req.ctx.audit!.inputDigest = { stepId: params.stepId };
  ...
  app.get("/governance/workflow/steps/:stepId/compensations", async (req) => {
    ...
    const params = z.object({ stepId: z.string().uuid() }).parse(req.params);
    setAuditContext(req, { resourceType: "governance", action: "workflow.step.compensate" });
    req.ctx.audit!.policyDecision = await requirePermission({ req, ...PERM.GOVERNANCE_WORKFLOW_STEP_COMPENSATE });
  ...
  app.post("/governance/workflow/compensations/:compensationId/retry", async (req) => {
    ...
    const decision = await requirePermission({ req, ...PERM.GOVERNANCE_WORKFLOW_STEP_COMPENSATION_RETRY });
    req.ctx.audit!.policyDecision = decision;
    ...
    const stepRes = await app.db.query(
      "SELECT step_id FROM steps WHERE run_id = $1 AND status IN ('failed','deadletter') ORDER BY seq ASC LIMIT 1",
      [runId],
    );
  ...
  app.post("/governance/workflow/compensations/:compensationId/cancel", async (req) => {
    ...
    const decision = await requirePermission({ req, ...PERM.GOVERNANCE_WORKFLOW_STEP_COMPENSATION_CANCEL });
    req.ctx.audit!.policyDecision = decision;
    req.ctx.audit!.inputDigest = { compensationId: params.compensationId };
  ...
  app.post("/governance/workflow/deadletters/:stepId/retry", async (req) => {
    ...
    const decision = await requirePermission({ req, ...PERM.GOVERNANCE_WORKFLOW_DEADLETTER_RETRY });
    req.ctx.audit!.policyDecision = decision;
  ...
  app.post("/governance/workflow/deadletters/:stepId/cancel", async (req) => {
    ...
    const decision = await requirePermission({ req, ...PERM.GOVERNANCE_WORKFLOW_DEADLETTER_CANCEL });
    req.ctx.audit!.policyDecision = decision;
