...
function getRequiredApprovals(inputDigest: any): number {
  const requireDualApproval = Boolean(inputDigest?.approvalPolicy?.requireDualApproval);
  return requireDualApproval ? 2 : 1;
}
