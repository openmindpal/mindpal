...
import { pageDraftSchema } from "../uiConfig/pageModel";
import { cloneReleasedVersion, getDraft as getPageDraft, getLatestReleased as getLatestReleasedPage, publishFromDraft as publishPageFromDraft, rollbackToPreviousReleased as rollbackPageToPreviousReleased, setPageVersionStatus } from "../uiConfig/pageRepo";
import { sha256Hex } from "../notifications/digest";
import { clearActiveVersion as clearWorkbenchActiveVersion, getActiveVersion as getWorkbenchActiveVersion, getCanaryConfig as getWorkbenchCanaryConfig, getDraftVersion as getWorkbenchDraft, getLatestReleasedVersion as getWorkbenchLatestReleased, getPreviousReleasedVersion as getWorkbenchPreviousReleased, publishFromDraft as publishWorkbenchFromDraft, rollbackActiveToPreviousReleased as rollbackWorkbenchActiveToPreviousReleased, setActiveVersion as setWorkbenchActiveVersion, setCanaryConfig as setWorkbenchCanaryConfig, clearCanaryConfig as clearWorkbenchCanaryConfig } from "../workbenches/workbenchRepo";
...
async function validateItem(pool: Pool | PoolClient, tenantId: string, item: ChangeSetItemRow) {
  if (item.kind === "tool.set_active") {
    ...
    if (ver.artifactRef) {
      ...
      if (!t.ok) throw new Error("trust_not_verified");
      if (!s.ok) throw new Error("scan_not_passed");
      if (!b.ok) throw new Error("sbom_not_present");
  ...
  if (item.kind === "model_limits.set") {
    const scopeType = String(item.payload?.scopeType ?? "");
    const scopeId = String(item.payload?.scopeId ?? "");
    const rpm = Number(item.payload?.modelChatRpm);
    if (scopeType !== "tenant" && scopeType !== "space") throw new Error("invalid_item");
    if (!scopeId) throw new Error("invalid_item");
    if (!Number.isFinite(rpm) || rpm <= 0 || rpm > 100000) throw new Error("invalid_item");
    return;
  }
...
function schemaDiffDigest(params: { prev: any | null; next: any }) {
  const prevEntities = (params.prev?.entities ?? {}) as Record<string, any>;
  const nextEntities = (params.next?.entities ?? {}) as Record<string, any>;
  const prevNames = new Set(Object.keys(prevEntities));
  const nextNames = new Set(Object.keys(nextEntities));
  ...
  let fieldsRemoved = 0;
  let requiredAdded = 0;
  const requiredAddedFields: string[] = [];
  let requiredUpgraded = 0;
  const requiredUpgradedFields: string[] = [];

  for (const n of nextNames) if (!prevNames.has(n)) entitiesAdded += 1;
  for (const n of prevNames) if (!nextNames.has(n)) entitiesRemoved += 1;
  ...
  for (const [entityName, nextEntity] of Object.entries(nextEntities)) {
    ...
    for (const f of nextFieldNames) {
      if (!prevFieldNames.has(f)) {
        fieldsAdded += 1;
        if (Boolean(nextFields[f]?.required)) {
          requiredAdded += 1;
          if (requiredAddedFields.length < 50) requiredAddedFields.push(`${entityName}.${f}`);
        }
      }
    }
    for (const f of prevFieldNames) if (!nextFieldNames.has(f)) fieldsRemoved += 1;
    for (const f of nextFieldNames) {
      if (!prevFieldNames.has(f)) continue;
      const prevRequired = Boolean(prevFields[f]?.required);
      const nextRequired = Boolean(nextFields[f]?.required);
      if (!prevRequired && nextRequired) {
        requiredUpgraded += 1;
        if (requiredUpgradedFields.length < 50) requiredUpgradedFields.push(`${entityName}.${f}`);
      }
    }
  }

  return { entitiesAdded, entitiesRemoved, fieldsAdded, fieldsRemoved, requiredAdded, requiredAddedFields, requiredUpgraded, requiredUpgradedFields };
}
...
export async function preflightChangeSet(params: { pool: Pool; tenantId: string; id: string; mode?: "full" | "canary" }) {
  ...
  for (const suiteId of requiredEvalSuites) {
    ...
    const reason = !latest ? "run:missing" : latest.status !== "succeeded" ? `run:${latest.status}` : passed ? null : "threshold:not_met";
    evals.push({ suiteId: suite.id, name: suite.name, passed, latestRunId: latest?.id ?? null, latestRunStatus: latest?.status ?? null, summary: latest?.summary ?? null, reason });
  }
  ...
  for (const item of items) {
    ...
    if (item.kind === "tool.enable" || item.kind === "tool.disable") {
      ...
      const trust = ver?.artifactRef ? toolTrustOk((ver as any).trustSummary) : { ok: true, status: "n/a" as const };
      const scan = ver?.artifactRef ? toolScanOk((ver as any).scanSummary) : { ok: true, mode: "n/a" as const, status: "n/a" as const };
      const sbom = ver?.artifactRef ? toolSbomOk({ sbomSummary: (ver as any).sbomSummary, sbomDigest: (ver as any).sbomDigest }) : { ok: true, mode: "n/a" as const, status: "n/a" as const };
      let isolationOk = true;
      const minIsoRaw = String(process.env.SKILL_ISOLATION_MIN ?? "").trim().toLowerCase();
      if (enabled && ver?.artifactRef && minIsoRaw === "remote") {
        const override = String(process.env.SKILL_RUNTIME_REMOTE_ENDPOINT ?? "").trim();
    ...
    if (item.kind === "tool.set_active") {
      ...
      let isolationOk = true;
      const minIsoRaw = String(process.env.SKILL_ISOLATION_MIN ?? "").trim().toLowerCase();
      if (ver?.artifactRef && minIsoRaw === "remote") {
        const override = String(process.env.SKILL_RUNTIME_REMOTE_ENDPOINT ?? "").trim();
        const runner = override ? { endpoint: override } : await getEnabledSkillRuntimeRunner({ pool: client(params.pool), tenantId: params.tenantId });
        isolationOk = Boolean(runner);
      }
    ...
    if (item.kind === "schema.publish") {
      ...
      if (mode === "canary") {
        for (const spaceId of targets) {
          ...
          const compat = checkSchemaCompatibility(prev?.schema ?? null, schemaDef);
          const diff = schemaDiffDigest({ prev: prev?.schema ?? null, next: schemaDef });
          const requiresMigration = diff.requiredAdded + diff.requiredUpgraded > 0;
          if (!compat.ok && !requiresMigration) warnings.push("schema_compat:failed");
          ...
          plan.push({
            ...
            nextVersionHint,
            ...diff,
            compatOk: compat.ok || requiresMigration,
            requiresMigration,
            migrationPlanDigest: requiresMigration
              ? { kind: "backfill_required_field", targetVersion: nextVersionHint, requiredAddedFields: diff.requiredAddedFields, requiredUpgradedFields: diff.requiredUpgradedFields }
              : null,
          });
    ...
    if (item.kind === "workbench.plugin.publish") {
      ...
      if (!draft) {
        ...
        contractChecks[contractChecks.length - 1] = { ...contractChecks[contractChecks.length - 1], contractKind: "workflow", status: "pass", errorCode: null, messageI18n: null, digest: (draft as any).manifestDigest ?? null };
      }
      plan.push({
        kind: item.kind,
        scopeType: cs.scopeType,
    ...
    if (item.kind === "model_limits.set") {
      ...
      if (mode === "canary") {
        if (scopeType !== "space" || !targets.includes(scopeId)) warnings.push("mode:canary_model_limits_not_supported");
      }
      const prevRes = await client(params.pool).query(
      ...
      plan.push({ kind: item.kind, scopeType, scopeId, modelChatRpm });
      currentStateDigest.push({ kind: "model.quota_limit", scopeType, scopeId, exists: Boolean(prev), prev });
      rollbackPreview.push({ kind: "model_limits.restore", scopeType, scopeId, exists: Boolean(prev) });
      continue;
    }
  ...
  return {
    ...
    evalGate: { requiredSuiteIds: requiredEvalSuites, suites: evals },
    plan,
    currentStateDigest,
    rollbackPreview,
    contractChecks,
...
export async function releaseChangeSet(params: { pool: Pool; tenantId: string; id: string; releasedBy: string; mode?: "full" | "canary" }) {
  ...
  if (isHighRisk && requiredEvalSuites.length) {
    for (const suiteId of requiredEvalSuites) {
      const suite = await getEvalSuite({ pool: params.pool, tenantId: params.tenantId, id: suiteId });
  ...
  try {
    ...
    for (const item of items) {
      ...
      if (item.kind === "schema.publish") {
        ...
        if (mode === "canary") {
          ...
          for (const spaceId of targets) {
            ...
            const diff = schemaDiffDigest({ prev: prev?.schema ?? null, next: schemaDef });
            const requiresMigration = diff.requiredAdded + diff.requiredUpgraded > 0;
            if (!compat.ok && !requiresMigration) throw new Error(`schema_compat_failed:${compat.reason}`);
            if (requiresMigration) {
              if (!migrationRunId) throw new Error("migration_required");
      ...
      if (item.kind === "model_limits.set") {
        ...
        const modelChatRpm = Number(item.payload.modelChatRpm);
        if (mode === "canary") {
          if (scopeType !== "space") throw new Error("changeset_mode_not_supported");
          if (!targets.includes(scopeId)) throw new Error("changeset_mode_not_supported");
        }
        const prevRes = await tx.query(
          `SELECT model_chat_rpm FROM quota_limits WHERE tenant_id = $1 AND scope_type = $2 AND scope_id = $3 LIMIT 1`,
          [params.tenantId, scopeType, scopeId],
        ...
        await tx.query(
          `
            ...
            DO UPDATE SET
              model_chat_rpm = EXCLUDED.model_chat_rpm,
              updated_at = now()
          `,
  ...
  try {
    ...
    for (const item of items) {
      ...
      if (item.kind === "model_limits.set") {
        ...
        const scopeId = String(item.payload.scopeId);
        const modelChatRpm = Number(item.payload.modelChatRpm);
        if (scopeType === "space" && cs.scopeType === "tenant") {
          const prevRes = await tx.query(
            `SELECT model_chat_rpm FROM quota_limits WHERE tenant_id = $1 AND scope_type = $2 AND scope_id = $3 LIMIT 1`,
            [params.tenantId, "tenant", cs.scopeId],
          );
          const prevRpm = prevRes.rowCount ? Number(prevRes.rows[0].model_chat_rpm) : null;
          rollback.actions.push({ kind: "model_limits.restore", scopeType: "tenant", scopeId: cs.scopeId, prevModelChatRpm: prevRpm });
          await tx.query(
            `
              INSERT INTO quota_limits (tenant_id, scope_type, scope_id, model_chat_rpm)
              VALUES ($1,$2,$3,$4)
            ...
            `,
            [params.tenantId, "tenant", cs.scopeId, modelChatRpm],
          );
        ...
        } else if (scopeType === "space" && cs.scopeType === "space" && scopeId === cs.scopeId) {
          const prevRes = await tx.query(
            `SELECT model_chat_rpm FROM quota_limits WHERE tenant_id = $1 AND scope_type = $2 AND scope_id = $3 LIMIT 1`,
            [params.tenantId, scopeType, scopeId],
          );
...
async function computeApprovalGate(params: { pool: Pool; tenantId: string; items: ChangeSetItemRow[] }) {
  ...
  return { riskLevel: risk, requiredApprovals };
}
