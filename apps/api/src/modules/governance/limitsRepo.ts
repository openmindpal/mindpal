...
function toQuota(r: any): QuotaLimit {
  return {
...
export async function getQuotaLimit(params: { pool: Pool; tenantId: string; scopeType: string; scopeId: string }) {
  ...
  return toQuota(res.rows[0]);
}

export async function upsertQuotaLimit(params: { pool: Pool; tenantId: string; scopeType: string; scopeId: string; modelChatRpm: number }) {
  const res = await params.pool.query(
    `
      ...
      RETURNING *
    `,
    [params.tenantId, params.scopeType, params.scopeId, params.modelChatRpm],
  );
  return toQuota(res.rows[0]);
}
