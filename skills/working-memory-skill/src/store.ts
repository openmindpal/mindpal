...
import { autoTtl, isNoExpiry, remainingTtl } from './ttl';
...
export class WorkingMemoryStore {
  ...
  async set(namespace: string, key: string, value: unknown, opts?: SetOptions): Promise<void> {
    const importance = opts?.importance ?? 0;
    const ttlMs = opts?.ttlMs ?? autoTtl(importance);
  ...
  async get(namespace: string, key: string): Promise<unknown | null> {
    ...
    if (l1Entry) {
      ...
      if (!isNoExpiry(l1Entry.ttlMs) && remainingTtl(l1Entry.createdAt, l1Entry.ttlMs) === 0) {
  ...
  async getMany(namespace: string, keys: string[]): Promise<Record<string, unknown>> {
  ...
  async scan(namespace: string, opts?: ScanOptions): Promise<MemoryEntry[]> {
    ...
    for (const [fullKey, entry] of this.l1.entries()) {
      ...
      if (!isNoExpiry(entry.ttlMs) && remainingTtl(entry.createdAt, entry.ttlMs) === 0) continue;
      if (opts?.tags && opts.tags.length > 0) {
        if (!entry.tags || !opts.tags.some(t => entry.tags!.includes(t))) continue;
      }
      if (opts?.minImportance !== undefined && (entry.importance ?? 0) < opts.minImportance) continue;
      results.push(entry);
    }
  ...
  async promote(namespace: string, key: string, targetType: 'episodic' | 'semantic' | 'procedural'): Promise<{ promoted: boolean; entry?: MemoryEntry }> {
    const fullKey = this.buildKey(namespace, key);
    ...
    if (!entry) {
      ...
      if (this.redisAvailable && this.redis) {
        const data = await this.redis.get(this.redisKey(namespace, key));
        if (data) {
          const redisEntry: MemoryEntry = JSON.parse(data);
  ...
  private tagIndexKey(namespace: string, tag: string): string {
  ...
  private startTtlSweep(): void {
    this.sweepTimer = setInterval(() => {
      ...
      for (const [key, entry] of this.l1.entries()) {
        if (!isNoExpiry(entry.ttlMs) && remainingTtl(entry.createdAt, entry.ttlMs) === 0) {
          this.l1.delete(key);
        }
      }
      this.stats.l1Size = this.l1.size;
