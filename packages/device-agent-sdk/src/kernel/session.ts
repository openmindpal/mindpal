 ...
 * @layer kernel
 *
 * [SDK迁移] 原 `from "../plugins/capabilityProbe"` 跨层依赖已解耦：
 * DeviceCapabilityReport 类型已提升到 kernel/types.ts；
 * getCachedCapabilityReport 通过可注入回调替代，SDK 不直接依赖插件层。
 */
import { exportCapabilityManifest } from "./capabilityRegistry";
import { exportMetricsSnapshot } from "./toolMetrics";
import type { DeviceCapabilityReport } from "./types";
...
// ── 可注入的能力报告提供者 ──────────────────────────────────
// [SDK迁移] 替代原 `getCachedCapabilityReport` 硬依赖
let _capabilityReportProvider: (() => DeviceCapabilityReport | null) | null = null;

/** 设置能力探测报告提供者（由应用层注入） */
export function setCapabilityReportProvider(fn: (() => DeviceCapabilityReport | null) | null): void {
  _capabilityReportProvider = fn;
}

function getCapabilityReport(): DeviceCapabilityReport | null {
  return _capabilityReportProvider?.() ?? null;
}
...
export async function sendHeartbeat(sendFn?: (body: any) => Promise<any>): Promise<boolean> {
  ...
  try {
    ...
    const res = await sender({ os: heartbeatConfig.os || "unknown", agentVersion: heartbeatConfig.agentVersion || "unknown", timestamp: new Date().toISOString(), sessions: sessionSummary, status: "active", capabilitySnapshot: exportCapabilityManifest(), metricsSnapshot: exportMetricsSnapshot(), deviceCapabilityReport: getCapabilityReport() ?? undefined });
...
export function updateSessionMetadata(sessionId: string, metadata: Record<string, any>): boolean {
  const session = sessions.get(sessionId);
  if (!session) return false;
  ...
  private async doSendHeartbeat(): Promise<boolean> {
    ...
    try {
      const res = await this.heartbeatSender({
        ...
        deviceCapabilityReport: getCapabilityReport() ?? undefined,
      });
