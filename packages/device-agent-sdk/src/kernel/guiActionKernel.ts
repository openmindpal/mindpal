 ...
 *
 * [SDK迁移] 原 `from "../plugins/localVision"` 跨层依赖已解耦：
 * 通过 setNativeGuiProvider 注入底层 GUI 操作实现，SDK 不直接依赖插件层。
 */
...
// ── 可注入的 native GUI 操作提供者 ─────────────────────────────

/** Native GUI 操作接口（由应用层注入实际的 localVision 实现） */
export interface NativeGuiProvider {
  clickMouse(x: number, y: number, button?: "left" | "right"): Promise<void>;
  doubleClick(x: number, y: number): Promise<void>;
  typeText(text: string): Promise<void>;
  pressKey(key: string): Promise<void>;
  pressCombo(keys: string[]): Promise<void>;
  moveMouse(x: number, y: number): Promise<void>;
  scroll(direction: "up" | "down", clicks: number): Promise<void>;
}

let _nativeGuiProvider: NativeGuiProvider | null = null;

/**
 * 设置 native GUI 操作提供者。
 * 应用层应在启动时调用此函数，注入实际的 localVision 实现。
 */
export function setNativeGuiProvider(provider: NativeGuiProvider | null): void {
  _nativeGuiProvider = provider;
}

function requireProvider(): NativeGuiProvider {
  if (!_nativeGuiProvider) throw new Error("native_gui_provider_not_set: call setNativeGuiProvider() first");
  return _nativeGuiProvider;
}
...
export async function executeNativeGuiAction(
  ...
  switch (action) {
    case "click": {
      ...
      await p.clickMouse(target.x, target.y, (params?.button ?? "left") as "left" | "right");
      detail.x = target.x;
      detail.y = target.y;
    ...
    case "doubleClick": {
      ...
      await p.doubleClick(target.x, target.y);
      detail.x = target.x;
      detail.y = target.y;
      break;
    }
    case "type": {
      if (target) {
        await p.clickMouse(target.x, target.y);
        await sleep(params?.typeClickDelayMs ?? 50);
        detail.x = target.x;
        detail.y = target.y;
      }
      const text = params?.text ?? "";
      await p.typeText(text);
    ...
    case "scroll": {
      ...
      await p.scroll(direction, clicks);
    ...
    case "moveTo": {
      ...
      await p.moveMouse(target.x, target.y);
      detail.x = target.x;
      detail.y = target.y;
      break;
    }
