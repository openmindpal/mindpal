 ...
 *
 * [SDK迁移] 跨层依赖已解耦：
 * - `../log` → 改为 `./log`（SDK 内核本地日志模块）
 * - `../plugins/builtinToolPlugin` → 通过 setBuiltinToolPlugin 注入，SDK 不硬编码应用层插件
 */
import type { ToolExecutionContext, ToolExecutionResult, DeviceClaimEnvelope, DeviceToolPlugin } from "./types";
...
// ══════════════════════════════════════════════════════════════
// 可注入的内置工具插件（替代原硬编码 builtinToolPlugin 导入）
// ══════════════════════════════════════════════════════════════

let _builtinToolPlugin: DeviceToolPlugin | null = null;

/**
 * 设置内置工具插件（noop/echo），由应用层注入。
 * SDK 不硬编码应用层插件引用。
 */
export function setBuiltinToolPlugin(plugin: DeviceToolPlugin | null): void {
  _builtinToolPlugin = plugin;
}
...
function ensureBuiltinToolPlugin(toolName: string): void {
  ...
  if (!_builtinToolPlugin) return; // SDK 模式下未注入则跳过
  try {
    registerPlugin(_builtinToolPlugin);
  } catch (err: any) {
