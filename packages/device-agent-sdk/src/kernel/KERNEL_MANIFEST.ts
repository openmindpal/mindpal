...
const AUXILIARY_KERNEL_FILES = ["types.ts", "config.ts", "index.ts", "log.ts", "toolFeatureFlags.ts", "toolMetrics.ts", "pluginSandbox.ts", "ocrCacheService.ts", "guiActionKernel.ts"];
...
export function validateKernelManifest(baseDir = path.resolve(__dirname, "..")): BoundaryValidationIssue[] {
  ...
  if (fs.existsSync(kernelDir)) {
    for (const entry of fs.readdirSync(kernelDir, { withFileTypes: true })) {
      ...
      // Skip build artifacts (.d.ts, .d.ts.map, .js compiled output)
      if (/\.d\.ts$/i.test(entry.name) || /\.d\.ts\.map$/i.test(entry.name)) continue;
      // Allow .js counterparts of known .ts files
      const tsName = entry.name.replace(/\.js$/i, ".ts");
      if (entry.name.endsWith(".js") && allowedFiles.has(tsName)) continue;
